...
Itigo::Itigo(const vvv &model, bool use_flip,
...
void Itigo::ExecuteOrWait(const std::vector<Command> commands) {
  ce->Execute(FillWait(commands));
}

void Itigo::FissionAndGo(int bid, const Point& goal, int m) {
...
void Itigo::DoHaiti(int x_num, int z_num) {
  CHECK_GT(x_num, 0);
  CHECK_GT(z_num, 0);
...
int Itigo::DoMove(const std::vector<std::pair<int, Point>>& targets) {
  int cnt = 0;
  while (true) {
    ++cnt;
...
int main(int argc, char** argv) {
  ...
  auto ai = std::make_unique<Itigo>(voxels, FLAGS_flip, FLAGS_x_num,
                                    FLAGS_z_num);
  ai->Run();
  ai->Finalize();
}
